...
인원수가 3명밖에 안 돼서 그냥 하드코딩했다.
하지만 쉬운 문제더라도 나중에 하드코딩으로 인한 실수를 줄이기 위해,
`i%5`, `i%8`... 대신 배열 길이인 `len(num1)` 과 같은 방식을 사용하는 습관을 들여야겠다.
또한 시간 복잡도적인 측면에서 answer를 clear하는 게 더 나은지, 아니면 해당 타겟 인덱스를 찾아가는 것이 더 나은지는 문제마다 다를 것 같다고 생각했다.

### 성능 요약

메모리: 9.25 MB, 시간: 3.98 ms
