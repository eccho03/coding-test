...
1.
`fireball`을 딕셔너리 말고 리스트로,
`arr = [[[] for _ in range(N)] for _ in range(N)]` 이렇게 관리하는 게 편리했던 문제였다.
이걸 바로 눈치채는 연습을 하자.

2.격자의 행과 열은 1번부터 N번까지 번호가 매겨져 있고, 1번 행은 N번과 연결되어 있고, 1번 열은 N번 열과 연결되어 있다.
이게 무슨 말인지 잘 이해가 안 됐다... 하지만 당연하게도 `s`가 클 때 배열 밖을 벗어날 수밖에 없으니 이렇게 떠올리기
```Python
ni,nj = (ci+di*s)%N, (cj+dj*s)%N
```

3. `all` 함수 숙지 - 이거를 몰라서 복잡하게 짜느라 코드가 꼬였었다.
```Python
...
4. 문제 제대로 읽기
홀수일 경우 1,3,5,7이고 짝수일 경우 0,2,4,6으로 착각했다.

코드 복잡해질수록 풀 확률이 낮아지니 최대한 단순하고 간결하게 짜기!!

### 성능 요약

메모리: 163952 KB, 시간: 396 ms
