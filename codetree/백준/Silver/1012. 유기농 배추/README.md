...
만약 재귀를 사용해서 풀어야 하는 문제라면, 위 코드를 상단에 필수적으로 써야 한다. 파이썬의 기본 재귀 깊이 제한은 `1000`으로 매우 얕은 편이다. 코딩테스트 환경에서는 에러 메시지를 볼 수 없다.
```Python
import sys
